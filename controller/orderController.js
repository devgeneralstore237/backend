...
const getAllOrders = async (req, res) => {
  ...
  try {
    ...
    const orders = await Order.find(queryObject)
      .select(
        "_id invoice paymentMethod subTotal total user_info dealer_info discount shippingCost status createdAt updatedAt"
      )
      .sort({ updatedAt: -1 })
      .skip(skip)
      .limit(limits);
...
const updateOrder = (req, res) => {
  ...
  const newInvoiceStatus = req.body.invoice_status;
  Order.updateOne(
    {
      _id: req.params.id,
    },
