...
const makeid = (length) => {
  let result = '';
  const characters = 'ABCDEFGHJKLMNPRSTUVWXY3456789';
  ...
  try {
    ...
    if (isAdded) {
      ...
      const newStaff = new Admin({
        ...
        role: req.body.role,
        dealerCode: req.body.dealerCode ?? makeid(5),
...
const forgetPassword = async (req, res) => {
  ...
  if (!isAdded) {
    ...
    const body = {
      ...
      <p>A request has been received to change the password for your <strong>GeneralStore</strong> account </p>
        ...
        <strong>GeneralStore Team</strong>
             `,
    };
...
const getAllStaff = async (req, res) => {
  ...
  try {
    const admins = await Admin.find({}, { password: 0  }).sort({ _id: -1 });
    res.send(admins);
  } catch (err) {
    res.status(500).send({
      message: err.message,
